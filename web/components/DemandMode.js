...
function EventCard({ e }) {
  return (
    <a href={e.url || undefined} target={e.url ? "_blank" : undefined} rel="noreferrer" className={`block rounded-lg border border-slate-100 p-3 ${e.url ? "hover:border-teal-300" : ""}`}>
      ...
      <div className="text-[11px] text-slate-400">{e.start} ~ {e.end}{typeof e.d === "number" ? ` · ${e.d}km` : ""}</div>
    </a>
  );
}

export default function DemandMode({ sidos, regions, sel, filterSido, setFilterSido, goRegion, step, setStep, nearEvents, allEvents, evFar, diag, TGT, getAppeal, doAppeal, onChangeRole }) {
    ...
    return (
      <div className="fixed inset-0 z-20 flex items-center justify-center bg-gradient-to-br from-slate-100 to-teal-50 px-5">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
          <button onClick={onChangeRole} className="mb-3 text-[12px] font-semibold text-slate-400 hover:text-slate-600">← 역할 선택으로</button>
    ...
    <div className="fixed inset-0 z-20 overflow-y-auto bg-slate-50 pt-5">
      <div className="mx-auto max-w-3xl px-5 pb-16">
        <div className="flex items-start justify-between">
          <div>
            <div className="text-[12px] text-slate-400">{sel?.sido} · 우리 동네</div>
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-900">{sel?.name}</h2>
          </div>
          <div className="flex gap-1.5">
            <button onClick={() => setStep("setup")} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[12px] font-semibold text-slate-600 hover:border-teal-300">지역 변경</button>
            <button onClick={onChangeRole} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[12px] font-semibold text-slate-600 hover:border-teal-300">← 역할</button>
          </div>
        </div>
        ...
        {view === "home" ? (
          <>
            <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <p className="mb-3 mt-0.5 text-[12px] text-slate-500">{evFar ? "40km 내 행사가 없어 가장 가까운 행사를 보여드려요." : "가까운 순으로 보여드려요."}</p>
              ...
              ) : <p className="rounded-lg bg-slate-50 p-3 text-[12px] text-slate-500">표시할 행사를 불러오는 중…</p>}
            </section>
            ...
            <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <p className="mb-3 mt-0.5 text-[12px] text-slate-500">부족한 대상의 교육이 필요하면 ‘수요 알리기’로 신호를 보내세요.</p>
              <div className="space-y-1.5">
                {(diag ? diag.top : []).slice(0, 5).map((x) => (
                  <div key={x.t} className="flex items-center justify-between rounded-lg border border-slate-100 p-2.5 text-[13px]">
                    ...
                    <button onClick={() => doAppeal(x.t)} className="rounded-full bg-teal-600 px-3 py-1 text-[12px] font-bold text-white hover:bg-teal-700">수요 알리기 {getAppeal(x.t)}</button>
                  </div>
                ))}
          ...
          <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <p className="mb-3 mt-0.5 text-[12px] text-slate-500">관심 있는 지역의 행사도 찾아보세요.</p>
            ...
            <div className="mb-2 text-[11px] text-slate-400">{otherList.length}건{oSido ? ` · ${oSido}` : ""}</div>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{otherList.slice(0, 30).map((e, i) => <EventCard key={i} e={e} />)}</div>
            {otherList.length === 0 && <p className="rounded-lg bg-slate-50 p-3 text-[12px] text-slate-500">해당 조건의 행사가 없어요.</p>}
          </section>
        )}
      </div>
    </div>
